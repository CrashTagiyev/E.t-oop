...
public:
	...
	void printQuestionAndRandomAnswersToTheFile() {
		...
		static fstream questionsFile(R"(C:\Users\Crash\Desktop\Questions.txt)", ios::out);
...
public:
	...
	void DefaultQuestions() {
		...
		{
			GeographyQuestions[i].printQuestionAndRandomAnswersToTheFile();
		}
		...
		for (size_t i = 0; i < CPPQuestions.size(); i++)
		{
			CPPQuestions[i].printQuestionAndRandomAnswersToTheFile();
		}
		...
		for (size_t i = 0; i < ArihmeticQuestions.size(); i++)
		{
			ArihmeticQuestions[i].printQuestionAndRandomAnswersToTheFile();
		}
	...
	//	for (size_t i = 0; i < GeographyQuestions.size(); i++)
	//	{
	//		int choice = interfaceController(i);
	...
	void start() {
		...
		if (choice == 0)
		{
			...
			case AdminEnum::CreateQUizz:quizzProgram.createQuiz();
				break;
			case AdminEnum::StartQuizz:cout << "sa";/*quizzProgram.startQuizz()*/;
				break;
			...
			{
				quizzProgram.startQuizz(_Guests[guestChoice], guestTypeChoice);
			}
